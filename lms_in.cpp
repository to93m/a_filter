...
int main(int argc, char *argv[]){
  ...
  temp_wave = new short[input_size + 1];
  for(int i = 0; i < input_size + L; i++) target_wave[i] = 0;
  read_wave(file_name.c_str(), temp_wave, target_size);
  for(int i = 0; i < input_size; i++){
    target_wave[i + (L / 2)] = temp_wave[i];
  }
  ...
  for(int iteration = 0; iteration < 7000; iteration++){ //iteration回数の設定
    for(int i = 0; i < input_size;i++){
      output = 0.0;
      for(int j = 0; j < L; j++){
        if(i - j >= 0){
